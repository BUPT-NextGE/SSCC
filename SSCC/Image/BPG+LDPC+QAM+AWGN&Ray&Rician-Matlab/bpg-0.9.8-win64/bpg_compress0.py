...
root_dir ='E:\\stl10\\1BPG_protection\\1CIFAR_test'    #测试集图像路径
compress_bpg_root = 'E:\\stl10\\1BPG_protection\\1CIFAR_q50_txt'   #为bpg压缩后的图像建立文件夹
# root_dir ='E:\\stl10\\1BPG_protection\\symbol_test'    #测试集图像路径
# compress_bpg_root = 'E:\\stl10\\1BPG_protection\\symbol_test_txt'   #为bpg压缩后的图像建立文件夹
test_bpg_len = 0
...
for item in os.listdir(root_dir):
        ...
        for pic_name in os.listdir(sub_file):
                ...
                compress_dir = bpg_file + '\\' + pic_name.replace('.jpg', '')   #压缩后的图像路径

                #q取值[1,51]，数值越高，压缩率越大，图像质量越差
                #os.system运行时cmd窗口不停闪烁
                #os.system('bpgenc -m 9 -b 8 -q 10 ' + file_name + ' -o ' + compress_dir + '.bpg')
                run('bpgenc -m 9 -b 8 -q 50 ' + file_name + ' -o ' + compress_dir + '.bpg',shell=True)
