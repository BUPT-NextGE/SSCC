...
M = 64; %调制阶数，16QAM、64QAM
snr = 10; %awgn信道信噪比
sym_all = 0;
num_err = 0;

in_root_dir = 'E:/stl10/1BPG_protection/symbol_jpg_txt';   %要进行LDPC编码的比特文件
out_root_dir = ['E:/stl10/1BPG_protection/symbol_out_txt/4QAM_out_txt',num2str(snr),'dB'];   %经过信道编码、调制、awgn、解调、信道解码后的比特文件
mkdir(out_root_dir);
...
for i = 3 : length(subdir)
    ...
    for j = 1 : length(txt_list)
        ...
        %%%记录img_bit的前214位和后16位%%%
        header = img_bit(1:214,1);
        tailer = img_bit(len_bit-15:len_bit,1);
        ...
        %%%需要把decoded的前214和后16个比特替换%%%
        decoded(1:214,1) = header;
        decoded(len_bit-15:len_bit,1) = tailer;
