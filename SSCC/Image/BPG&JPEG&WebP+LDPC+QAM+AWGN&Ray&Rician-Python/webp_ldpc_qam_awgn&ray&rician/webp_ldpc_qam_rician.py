...
@author Echo
@description
对输入数据集图片模拟经过WebP压缩并进行LDPC信道编码 -> QAM调制 -> rician信道 -> QAM解调 -> LDPC信道解码最后恢复出图片的整套流程
"""
...
if __name__ == "__main__":
    ...
    # 输出JPEG图片数据集根父路径（未进行LDPC+QAM+rayleigh）
    output_base_path = 'E:\\stl10\\stl_webp'

    # 图片对应字节比特流txt的目录
    output_txt_path = 'E:\\stl10\\stl_webp_bit_rician'

    # 输出通过了信道传输的JPEG图片数据集根父路径（经过了LDPC+QAM+rayleigh）
    channelcoded_output_base_path = 'E:\\stl10\\stl_webp_with_ldpc_qam_rician'
    ...
    """
    读取对应txt得到比特流数组，并进行LDPC+QAM+rayleigh，最后恢复，得到经过WebP压缩并且通过了LDPC+QAM+rayleigh的图片
    """
