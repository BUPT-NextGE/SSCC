...
@author Echo
@description
对输入数据集图片模拟经过WebP压缩并进行LDPC信道编码 -> QAM调制 -> AWGN信道 -> QAM解调 -> LDPC信道解码最后恢复出图片的整套流程
"""
...
def pillow_encode(img, output_img_path, fmt='webp', quality=10):
...
def find_closest_bpp(target, img, dir, fmt='webp'):
    lower = 0
    upper = 100
    prev_mid = upper
    for i in range(10):
...
if __name__ == "__main__":
    ...
    # 输入图片数据集根父路径
    input_base_path = '/Users/serendipity/Downloads/stl'

    # 输入的图片格式(后缀名，区分大小写)
    input_fmt = 'png'

    # 输出WebP图片数据集根父路径（未进行LDPC+QAM+AWGN）
    output_base_path = '/Users/serendipity/Downloads/stl_webp'

    # 图片对应字节比特流txt的目录
    output_txt_path = '/Users/serendipity/Downloads/stl_webp_bit'

    # 输出通过了信道传输的WebP图片数据集根父路径（经过了LDPC+QAM+AWGN）
    channelcoded_output_base_path = '/Users/serendipity/Downloads/stl_webp_with_ldpc_qam_awgn'
    ...
    for img_dir in input_images:
        ...
        # 输出图片路径(指定到文件名)
        output_img_path = output_path + '/' + img_dir.split('/')[-1].split('.')[0] + '.webp'
        img = Image.open(img_dir)
        img = img.convert("RGB")
        bpp_per_img = find_closest_bpp(target_bpp, img, output_img_path, fmt='WEBP')
    ...
    """
    对WebP压缩后的图片读取字节流，并转换为比特流，存入txt
    """
    if not os.path.exists(output_txt_path):
        os.makedirs(output_txt_path)

    input_images = glob.glob(os.path.join(output_base_path, '**/*.webp'), recursive=True)
    for img_dir in input_images:
    ...
    """
    读取对应txt得到比特流数组，并进行LDPC+QAM+AWGN，最后恢复，得到经过WebP压缩并且通过了LDPC+QAM+AWGN的图片
    """
