...
def ldpc_qam_rician(input_signal, snr=2, qam_order=16,K=100):
...
if __name__ == "__main__":
    ...
    # 输出JPEG图片数据集根父路径（未进行LDPC+QAM+Rician）
    output_base_path = 'E:\\stl10\\stl_jpeg'

    # 图片对应字节比特流txt的目录
    output_txt_path = 'E:\\stl10\\stl_jpeg_bit'

    # 输出通过了信道传输的JPEG图片数据集根父路径（经过了LDPC+QAM+Rician）
    channelcoded_output_base_path = 'E:\\stl10\\stl_jpeg_with_ldpc_qam_rician'
    ...
    """
    读取对应txt得到比特流数组，并进行LDPC+QAM+Rician，最后恢复，得到经过JPEG压缩并且通过了LDPC+QAM+Rician的图片
    """
    ...
    for txt_dir in input_txts:
        ...
        output_signal = ldpc_qam_rician(input_signal, snr=snr, qam_order=qam_order,K=K)
        bitstring = ''
